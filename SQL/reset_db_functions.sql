...
END;
 ...
 DELIMITER //
 CREATE FUNCTION insert_locations (user_name_ varchar(60), password_ varchar(60), country_ varchar(60), state_ varchar(60), city_ varchar(60), longitude_ varchar(60), latitude_ varchar(60)) RETURNS INTEGER DETERMINISTIC
 BEGIN
 IF ((SELECT COUNT(`pk_user_id`) FROM USERS WHERE `user_name` = user_name_ AND `user_password`=password_) > 0)
 THEN
 INSERT INTO USER_LOCATIONS(country, state_province, city, date_, userid, longitude, latitude) VALUES(country_, state_, city_, NOW(), (SELECT `pk_user_id` FROM USERS WHERE `user_name` = user_name_ AND `user_password` = password_), longitude_, latitude_);
 RETURN 0;
 ELSE
 RETURN -1;
 END IF;
